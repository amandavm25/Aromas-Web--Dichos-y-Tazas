...
function initializeModals() {
    ...
    botonesDetalles.forEach(boton => {
        boton.addEventListener('click', function () {
            ...
            const datosAnteriores = this.getAttribute('data-datos-anteriores');
            const datosNuevos = this.getAttribute('data-datos-nuevos');
            const fecha = this.getAttribute('data-fecha');

            document.getElementById('detalles-id-bitacora').textContent = id;
            document.getElementById('detalles-empleado-bitacora').textContent = empleado;
            document.getElementById('detalles-modulo-badge-bitacora').textContent = modulo;
            document.getElementById('detalles-accion-bitacora').textContent = accion;
            document.getElementById('detalles-tabla-bitacora').textContent = tabla || 'N/A';
            document.getElementById('detalles-descripcion-bitacora').textContent = descripcion;
            document.getElementById('detalles-fecha-bitacora').textContent = fecha;

            // Formatear y mostrar datos anteriores
            const datosAnterioresEl = document.getElementById('detalles-datos-anteriores-bitacora');
            if (datosAnteriores && datosAnteriores !== 'null' && datosAnteriores !== '') {
                try {
                    const datosFormateados = JSON.stringify(JSON.parse(datosAnteriores), null, 2);
                    datosAnterioresEl.textContent = datosFormateados;
                } catch (e) {
                    datosAnterioresEl.textContent = datosAnteriores;
                }
            } else {
                datosAnterioresEl.textContent = 'Sin datos anteriores';
            }

            // Formatear y mostrar datos nuevos
            const datosNuevosEl = document.getElementById('detalles-datos-nuevos-bitacora');
            if (datosNuevos && datosNuevos !== 'null' && datosNuevos !== '') {
                try {
                    const datosFormateados = JSON.stringify(JSON.parse(datosNuevos), null, 2);
                    datosNuevosEl.textContent = datosFormateados;
                } catch (e) {
                    datosNuevosEl.textContent = datosNuevos;
                }
            } else {
                datosNuevosEl.textContent = 'Sin datos nuevos';
            }
        });
    });
}
